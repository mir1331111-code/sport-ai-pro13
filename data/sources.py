"""data/sources.py — TheSportsDB основной источник + football-data.org."""
...
from config import (CACHE_TTL, FOOTBALL_DATA_ORG_HOST,
                    ...
                    WOMEN_LEAGUE_IDS, NATIONAL_TEAM_LEAGUES)
...
def _msk_date(utc_iso: str) -> str:
    try:
        ...
        return (dt + timedelta(hours=MSK_OFFSET_HOURS)).strftime("%Y-%m-%d")
    except Exception:
...
def _msk_time(utc_iso: str) -> str:
    try:
        dt = datetime.strptime(utc_iso[:19], "%Y-%m-%dT%H:%M:%S")
        return (dt + timedelta(hours=MSK_OFFSET_HOURS)).strftime("%H:%M")
    except Exception:
...
# ============ ИСТОРИЯ ДЛЯ ОБУЧЕНИЯ (football-data.co.uk CSV) ============
def load_seasonal(div: str, season: str) -> list:
    ...
    try:
        ...
        out = [row for row in csv.DictReader(io.StringIO(text))
               ...
               and row.get("FTHG") not in (None, "") and row.get("FTAG") not in (None, "")]
        cache_put(ck, out); return out
    except Exception: return []


# ============ THE SPORTS DB — ОСНОВНОЙ ИСТОЧНИК МАТЧЕЙ ============
def tsdb_matches(days: int = 7, logs=None) -> list:
    """Все матчи: мужские клубные + женские + сборные. Бесплатно, без ключа."""
    today = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
    
    # Строим обратную карту: TSDB_ID → (div_code, league_name, kind)
    reverse_map = {}
    for div_code, tsdb_id in DIV_TO_TSDB.items():
        reverse_map[str(tsdb_id)] = (div_code, None, "club")
    for tsdb_id, (div_code, league_name) in WOMEN_LEAGUE_IDS.items():
        reverse_map[str(tsdb_id)] = (div_code, league_name, "women")
    for tsdb_id, (div_code, comp_name) in NATIONAL_TEAM_LEAGUES.items():
        reverse_map[str(tsdb_id)] = (div_code, comp_name, "national")
    
    out = []
    stats = {"club": 0, "women": 0, "national": 0}
    
    for off in range(days):
        d = today + timedelta(days=off)
        dstr = d.strftime("%Y-%m-%d")
        ck = f"tsdb_all_v2_{dstr}"
        ...
        try:
            ...
            for e in events:
                ...
                div_code, custom_name, kind = reverse_map[league_id]
                h = e.get("strHomeTeam"); a = e.get("strAwayTeam")
                if not h or not a: continue
                
                rows.append({
                    "Div": div_code,
                    "League": custom_name or e.get("strLeague") or div_code,
    ...
    if logs:
        logs.append(f"📡 TheSportsDB: всего {len(out)} · 🏟 клубные {stats['club']} · 👩 женские {stats['women']} · 🌍 сборные {stats['national']}")
...
def _fdorg_get(endpoint: str, params: dict, token: str) -> Optional[dict]:
    ...
    try:
        r = _sess.get(url, headers=headers, params=params, timeout=15, proxies=NO_PROXY)
        usage.fdorg_increment(1)
        if r.status_code == 429:
            import time; time.sleep(60); return None
...
def fdorg_matches(days: int, token: str, logs=None) -> list:
    ...
    for comp in list(DIV_TO_FDORG.values()):
        if usage.fdorg_remaining() <= 0:
            if logs: logs.append("fdorg: лимит"); break
        div_code = FDORG_TO_DIV.get(comp, "G")
        ck = f"fdorg_v3_{comp}_{d_from}_{d_to}"
        cached = cache_get(ck, 1800)
        if cached is not None:
            if isinstance(cached, list): out += cached
            if logs: logs.append(f"OK {comp}: {len(cached) if isinstance(cached,list) else 0} (cached)")
            continue
        data = _fdorg_get("matches", {"dateFrom": d_from, "dateTo": d_to, "competitions": comp}, token)
        if not data or not data.get("matches"):
            if logs: logs.append(f"-- {comp}: 0")
            cache_put(ck, []); continue
        rows = []
        for m in data["matches"]:
            try:
                ...
                rows.append({
                    "Div": div_code, "League": (m.get("competition") or {}).get("name") or comp,
                    "competition": comp, "Date": _msk_date(utc_date), "Time": _msk_time(utc_date),
                    "HomeTeam": home, "AwayTeam": away, "fixture_id": m.get("id"),
...
def fdorg_match_result(match_id, token: str) -> Optional[dict]:
    ...
    if not data or not data.get("match"): return None
    m = data["match"]
...
def odds_api_fixture(sport_key: str, home: str, away: str, api_key: str) -> Optional[dict]:
    ...
    if usage.odds_remaining() <= 0: return None
    try:
        ...
        target = None
        for ev in events:
            eh = _norm_name(ev.get("home_team", "")); ea = _norm_name(ev.get("away_team", ""))
            if eh == hn and ea == an: target = ev; break
            if (eh in hn or hn in eh) and (ea in an or an in ea): target = ev; break
        ...
        for bmk in target.get("bookmakers") or []:
            for mkt in bmk.get("markets") or []:
                ...
                for out in mkt.get("outcomes") or []:
                    on = out.get("name") or ""; odd = _f(out.get("price"))
        ...
        if not (("П1" in out and "X" in out and "П2" in out) or
                ...
                ("BTTS да" in out and "BTTS нет" in out)): out = {}
        cache_put(ck, out or None); return out or None
    except Exception: return None

def team_logo_url(team_name: str) -> Optional[str]: return None
def league_logo_url(div_code: str) -> Optional[str]: return None
