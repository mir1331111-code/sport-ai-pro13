...
def render():
    ...
    clv_avg = float(clv.get("avg_clv", 0.0) or 0.0)
    ...
    if len(banks) >= 2:
        ...
        cur_peak = max(banks)
        if cur_peak > 0:
            cur_dd_pct = (cur_peak - banks[-1]) / cur_peak
    ...
    c1.metric("CLV avg", f"{clv_avg * 100:+.2f}%")
    c2.metric("CLV N", clv_n)
    ...
    if len(hist) > 1:
        try:
            ...
            if "ts" in df.columns and "bank" in df.columns:
